...
options { tokenVocab = MiniCSLexer; }

program
    : usingDecl* CLASS ident BL ( varDecl | classDecl | methodDecl )* BR
    ;
...
classDecl
    : CLASS ident BL ( varDecl | methodDecl )* BR
    ;

methodDecl
    : ( type | VOID ) ident LEFTP ( formPars )? RIGHTP block
    ;
...
type
    : simpleType ( SBL SBR )?          # simpleOrArray
...
statement
    : designator ( ASSIGN expr
                 | LEFTP ( actPars )? RIGHTP
                 | ADD
                 | SUB
                 ) SEMICOLON           # assignStatement
...
factor
    : designator ( LEFTP ( actPars )? RIGHTP )?   # designatorFactor
    | NUMLIT                                       # numLitFactor
    | FLOATLIT                                     # floatLitFactor
    | CHARLIT                                      # charLitFactor
    | STRINGLIT                                    # stringLitFactor
    | TRUE                                         # trueFactor
    | FALSE                                        # falseFactor
    | NEW type ( SBL ( expr )? SBR )?              # newObjectOrArray
    | LEFTP expr RIGHTP                            # parenExpr
    | listLiteral                                  # listLiteralFactor
    ;

listLiteral
    : LESS expr ( COMMA expr )* GREATER
    ;

designator
    : ident ( DOT ident | SBL expr SBR )*
    ;
...
caseBlock
    : CASE condition COLON statement*
    ;
